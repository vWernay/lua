...
namespace glm {
  ...
  struct Ray {
    ...
    Point dir;  // The normalized direction vector of this ray. [similarOverload: pos]
  ...
  template<length_t L, typename T, qualifier Q>
  GLM_FUNC_QUALIFIER GLM_CONSTEXPR bool equal(Ray<L, T, Q> const &x, Ray<L, T, Q> const &y, T Epsilon = epsilon<T>()) {
    return all(equal(x.pos, y.pos, Epsilon)) && all(equal(x.dir, y.dir, Epsilon));
  }

  template<length_t L, typename T, qualifier Q>
  GLM_FUNC_QUALIFIER GLM_CONSTEXPR bool equal(Ray<L, T, Q> const &x, Ray<L, T, Q> const &y, vec<L, T, Q> const &Epsilon) {
    return all(equal(x.pos, y.pos, Epsilon)) && all(equal(x.dir, y.dir, Epsilon));
  }

  template<length_t L, typename T, qualifier Q>
  GLM_FUNC_QUALIFIER GLM_CONSTEXPR bool equal(Ray<L, T, Q> const &x, Ray<L, T, Q> const &y, int MaxULPs) {
    return all(equal(x.pos, y.pos, MaxULPs)) && all(equal(x.dir, y.dir, MaxULPs));
  }

  template<length_t L, typename T, qualifier Q>
  GLM_FUNC_QUALIFIER GLM_CONSTEXPR bool equal(Ray<L, T, Q> const &x, Ray<L, T, Q> const &y, vec<L, int, Q> const &MaxULPs) {
    return all(equal(x.pos, y.pos, MaxULPs)) && all(equal(x.dir, y.dir, MaxULPs));
  }

  template<length_t L, typename T, qualifier Q>
  GLM_FUNC_QUALIFIER GLM_CONSTEXPR bool notEqual(Ray<L, T, Q> const &x, Ray<L, T, Q> const &y, T Epsilon = epsilon<T>()) {
    return any(notEqual(x.pos, y.pos, Epsilon)) || any(notEqual(x.dir, y.dir, Epsilon));
  }

  template<length_t L, typename T, qualifier Q>
  GLM_FUNC_QUALIFIER GLM_CONSTEXPR bool notEqual(Ray<L, T, Q> const &x, Ray<L, T, Q> const &y, vec<L, T, Q> const &Epsilon) {
    return any(notEqual(x.pos, y.pos, Epsilon)) || any(notEqual(x.dir, y.dir, Epsilon));
  }

  template<length_t L, typename T, qualifier Q>
  GLM_FUNC_QUALIFIER GLM_CONSTEXPR bool notEqual(Ray<L, T, Q> const &x, Ray<L, T, Q> const &y, int MaxULPs) {
    return any(notEqual(x.pos, y.pos, MaxULPs)) || any(notEqual(x.dir, y.dir, MaxULPs));
  }

  template<length_t L, typename T, qualifier Q>
  GLM_FUNC_QUALIFIER GLM_CONSTEXPR bool notEqual(Ray<L, T, Q> const &x, Ray<L, T, Q> const &y, vec<L, int, Q> const &MaxULPs) {
    return any(notEqual(x.pos, y.pos, MaxULPs)) || any(notEqual(x.dir, y.dir, MaxULPs));
  }
  ...
  namespace detail {
    ...
    struct compute_to_string<Ray<L, T, Q>> {
      GLM_FUNC_QUALIFIER static std::string call(const Ray<L, T, Q> &ray) {
